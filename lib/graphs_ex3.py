...
def make_baseline_graphs(utilities_test_rand,utilities_test_best,
    ...
    block_utilities_test_rand = np.mean(np.array(utilities_test_rand).reshape(25,-1),0)
    block_utilities_test_best = np.mean(np.array(utilities_test_best).reshape(25,-1),0)
    ...
    #block_rew_test = np.mean(np.array(utilities_test).reshape(25,-1),0)
    block_rew_test_rand = np.mean(np.array(rewards_test_rand).reshape(25,-1),0)
    block_rew_test_best = np.mean(np.array(rewards_test_best).reshape(25,-1),0)
...
def make_agent_graphs(rewards_test_best, rewards_test_rand, rewards_test, 
    ...
    block_utilities_test = np.mean(np.array(utilities_test).reshape(50,-1),0)
    block_utilities_test_rand = np.mean(np.array(utilities_test_rand).reshape(50,-1),0)
    block_utilities_test_best = np.mean(np.array(utilities_test_best).reshape(50,-1),0)


    block_wealth_test_rand = np.mean(np.array(wealth_test_rand).reshape(50,-1),0)
    block_wealth_test_best = np.mean(np.array(wealth_test_best).reshape(50,-1),0)
    block_wealth_test = np.mean(np.array(wealth_test).reshape(50,-1),0)

    
    block_rewards_test = np.mean(np.array(rewards_test).reshape(50,-1),0)
    block_rewards_test_rand = np.mean(np.array(rewards_test_rand).reshape(50,-1),0)
    block_rewards_test_best = np.mean(np.array(rewards_test_best).reshape(50,-1),0)
    ...
    plt.title('Distribution of Final rewards Merton v Trained Agent v Random (per 50 episodes)')
    plt.xlabel('Episode Rewards')
    plt.ylabel('Frequency')
    plt.legend()
    plt.show()
    ...
    sns.distplot(block_utilities_test_rand, label="Random agent")
    plt.title('Distribution of Final Utilities Merton v Trained Agent v Random (per 50 episodes)')
    ...
    plt.plot(np.convolve(utilities_test_best, np.ones((100,))/100, mode='valid'), label='Merton Optimal')
    plt.plot(np.convolve(utilities_test_rand, np.ones((100,))/100, mode='valid'), label='Random agent')
    plt.plot(np.convolve(utilities_test, np.ones((100,))/100, mode='valid'), label='Trained agent')
    plt.title('Moving average 100 episode utilities')
    plt.ylabel('Utility')
    plt.xlabel('Episode')
    plt.legend()
    plt.show()
    ...
    plt.title('Distribution of Final Wealth Merton v Trained Agent v Random (per 50 episodes)')
    ...
    plt.title('Distribution of Final Sharpe ratios  Merton v Trained Agent v Random (per 50 episodes)')
    ...
    ax.set_facecolor('white')
    ax.grid(0)
    plt.title("Violin plot of Sortino ratios (M period) - Random v Merton v Agent")
    plt.show()
    ...
    plt.title('Distribution of Final Sortino ratios  Merton v Trained Agent v Random (per 50 episodes)')
    plt.xlabel('Episode Sortino')
    plt.ylabel('Frequency')
    plt.legend()
    plt.show()
...
def plot_sample_paths(S,mu,sigma):
    ...
    plt.xlabel('Rebalance period - time')
...
def plot_disc_utility(u_star,utility,gamma=None):
    ...
    if gamma:
        ...
        plt.title("Simulated Discretised Log Utility")
        plt.ylabel("Log Utility")
    plt.xlabel("Investment Ratio")

def plot_mv_equiv(u_star, vals):
    plt.plot(u_star, vals)
    plt.title("Mean Variance equivalent")
    plt.xlabel("Investment Ratio")
    plt.ylabel("Mean-kappa/2*variance")

def plot_mv_equivs(u_star, vals, kappa):
    plt.plot(u_star, vals)
    plt.legend([str(kappa)])
    plt.title("Mean Variance equivalent")
    plt.xlabel("Investment Ratio")
    plt.ylabel("Mean-kappa/2*variance")
