...
np.random.seed(0)
class Market_sp500(object):
  def __init__(self, kappa, episodes, r, sigma, start, end,ticker):
    ...
    self.S, self.B, self.dS, self.dB = self.generate_price_series2()
  ...
  def step(self, action):
    ...
    if self.t < self.time_periods:
        ...
        log_reward = np.log(1+dX/wealth)  #try directly for log utility and see if any different
        done = False
    ...
    return reward,log_reward, discount, self.get_obs(), dX, done#reward, log_reward, discount,new_state, dX, done

  def generate_price_series2(self):
    I = self.episodes
    M = self.time_periods
    data = self.data
    S0 = 1
